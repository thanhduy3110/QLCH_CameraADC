...
namespace DAL
{
    public class HoaDonNhap_DAL
    {
        ...
        {
            return KetNoi.GetDataTable("select MaHDN,NV.HoTen,NgayLap,TongTien,HD.TrangThai From HoaDonNhap HD,NhanVien NV Where NV.MaNV=HD.MaNV  and HD.TrangThai=N'1'" + condition);
        }
...
values('" + ex.MaHDN + "','" + ex.MaSP + "'," + ex.MaNCC + "," + ex.SoLuong + "," + ex.GiaNhap + "," + ex.ThanhTien + ",N'"+ex.GhiChu+"'," + ex.TrangThai + ")");
        ...
        {
            KetNoi.ExecuteReader(@"update CTHD_Nhap Set MaNCC='"+ex.MaNCC+"',SoLuong=" + ex.SoLuong + ",GiaNhap=" + ex.GiaNhap + ",ThanhTien=" + ex.ThanhTien + ",GhiChu=N'"+ex.GhiChu+"'  Where MaHDN=N'" + ex.MaHDN +"'");
        }
